...
cronRouter.post('/run', async (req, res) => {
  ...
  let alertsCreated = 0, emailsSent = 0, pushesSent = 0;

  for (const u of users) {
    try {
      alertsCreated += await regenerateAlerts(u.user_id);

      // New, not-yet-notified alerts that warrant a nudge.
      ...
      const tokens = await query(`SELECT token FROM device_tokens WHERE user_id=$1`, [u.user_id]);
      if (tokens.length > 0) {
        const sent = await sendPush(
          tokens.map((t) => t.token),
          pending.length > 1 ? `PayWatch · ${pending.length} things need attention` : 'PayWatch',
          pending[0].title
        );
        if (sent > 0) { pushesSent += sent; notified = true; }
      }

      // Email digest to users who added an email.
      if (u.email) {
        ...
        if (ok) { emailsSent++; notified = true; }
      }

      if (notified) await query(`UPDATE notifications SET emailed_at=now() WHERE id = ANY($1::uuid[])`, [pending.map((a) => a.id)]);
    } catch (e) {
      console.error(`[cron] user ${u.user_id} failed`, e);
    }
  }

  res.json({ ok: true, users: users.length, alertsCreated, emailsSent, pushesSent });
});
